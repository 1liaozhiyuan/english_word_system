...
def format_word(word: WordRead) -> str:
    return (
        ...
        f"释义：{word.meaning}\n"
        f"英文释义：{word.english_definition or '无'}\n"
        f"词性：{word.part_of_speech or '无'}\n"
        f"词根词缀：{word.root_affix or '无'}\n"
        f"常见搭配：{word.collocations or '无'}\n"
        f"同义词：{word.synonyms or '无'}\n"
        f"反义词：{word.antonyms or '无'}\n"
        f"派生词：{word.word_family or '无'}\n"
        f"易混词：{word.confusing_words or '无'}\n"
        f"考试标签：{word.exam_tags or '无'}\n"
        f"难度标签：{word.difficulty_tag or '无'}\n"
